...
std::vector<double> conjugateGradient(const CCSMatrix& A, const std::vector<double>& b, const std::vector<double>& x_star, size_t maxIters) {
    ...
    std::ofstream outFile("residual_noprecond.txt");
    if (!outFile) {
...
// Method to convert CCSMatrix to Eigen::SparseMatrix
Eigen::SparseMatrix<double> toEigen(const CCSMatrix& ccsMatrix) {
    size_t n = ccsMatrix.size;
    Eigen::SparseMatrix<double> mat(n, n);

    for (int col = 0; col < n; ++col) {
        for (int i = ccsMatrix.JA[col]; i < ccsMatrix.JA[col + 1]; ++i) {
            int row = ccsMatrix.IA[i];
            double value = ccsMatrix.V[i];
            mat.insert(row, col) = value;

            // Since the matrix is symmetric, insert the mirrored value as well
            if (row != col) {
                mat.insert(col, row) = value;
            }
        }
    }
    return mat;
...
//function for CG with Diagonal Preconditioner
void ConjugateGradient_DP(Eigen::SparseMatrix<double>& A,Eigen::VectorXd & x_star,Eigen::VectorXd & b, int max_Iters, std::ofstream& outFile) {
    ...
    for (int i = 1; i <= max_Iters; i++)
    {
        ...
        x = cg_diagonal.solveWithGuess(b, x);
...
int main(int argc, char* argv[]) {
    ...
    //convert CCS Matrix in Eigen matrix
    Eigen::SparseMatrix<double> A = toEigen(mat);

    Eigen::VectorXd X_star = Eigen::Map<Eigen::VectorXd>(x_star.data(), x_star.size());
    Eigen::VectorXd B = Eigen::Map<Eigen::VectorXd>(b.data(), b.size());

    //CGDP outputfile
    std::ofstream outFileDP("residual_withprecond.txt",std::ios::app);
    if (!outFileDP.is_open()) {
        std::cerr << "Error opening output file." << std::endl;
        return 1;
    }
    
    if (outFileDP.tellp() == 0) {
        outFileDP << "Iteration\tResidual\n";
        
    }
    
    //CGDP
    ConjugateGradient_DP(A,X_star, B, opts.iters, outFileDP);
